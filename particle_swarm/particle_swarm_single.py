...
from particle_swarm.tests.linear_regression import boston as test
...
def pso(particles):
    ...
    while round(gbest_s, PRECISION) != TARGET_SCORE and run_count < MAX_ITERATIONS:
        # update scores
        particles["curr_score"] = [_score(particle["curr_pos"]) for particle in particles]
        ...
        vel_t = [_calc_vel_t(particle, gbest_p, r_1, r_2) for particle in particles]
